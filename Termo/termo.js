//Número de linhas total e tentativas
var height = 6;
//Comprimento da palavra
var width = 5;

//Linha e tentativa atual
var row = 0;
//Letra atual
var col = 0;

//Controle de quando o jogo finaliza
var gameOver = false;
...
//Apresentar a palavra para ajudar no teste do jogo
console.log(word);

//Função que é executada na hora que o arquivo é aberto. Semelhante ao Start() na Unity
window.onload = function ()
{
    initialize();
}

//Função que cria o jogo inicial
function initialize()
{
    //Criar o jogo
    for (let r = 0; r < height; r++) {
        for (let c = 0; c < width; c++) {
            ...
            tile.id = "jogo-" + r.toString() + '-' + c.toString();
...
function update() {
    ...
    //Primeira verificação: chegar as letras nas posições corretas
    for (let c = 0; c < width; c++) {
        let currTile = document.getElementById("jogo-" + row.toString() + '-' + c.toString());
        let letter = currTile.innerText;
    ...
    //Segunda verificação: quais letras estão presentes, mas em posições erradas
    for (let c = 0; c < width; c++) {
        let currTile = document.getElementById("jogo-" + row.toString() + '-' + c.toString());
        let letter = currTile.innerText;
    ...
    for (let c = 0; c < width; c++) {
        let currTile = document.getElementById("jogo-" + row.toString() + '-' + c.toString());
        currTile.innerText = word[c].toUpperCase();
    }
    row += 1;
    col = 0;
